...
fun SettingsScreen(
    ...
    Column(
        ...
        SettingsSection(title = "TikTok LIVE") {
            ...
            Text(
                "Not wired into the character's reactions yet (that's a later stage) - " +
                    "use \"Test connection\" below to confirm comments come through on this device.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
            Spacer(Modifier.height(14.dp))
            TikTokConnectionTestPanel(username = settings.tiktokUsername)
        }
...
/**
 * Lets you connect [com.ailivebear.app.tiktok.WebSocketTikTokCommentProvider]
 * to a real `wss://ws.adjdev.site` session and see comments arrive live, right
 * here in Settings - before anything in the app reacts to them. This panel is
 * a verification tool, not the real pipeline; a later stage feeds
 * WebSocketTikTokCommentProvider into CharacterManager directly instead.
 */
@Composable
private fun TikTokConnectionTestPanel(username: String) {
    val viewModel: com.ailivebear.app.tiktok.TikTokTestViewModel =
        viewModel(factory = com.ailivebear.app.tiktok.TikTokTestViewModelFactory())
    val state by viewModel.uiState.collectAsState()
    val isBusy = state.status == com.ailivebear.app.tiktok.TikTokConnectionStatus.CONNECTING ||
        state.status == com.ailivebear.app.tiktok.TikTokConnectionStatus.RECONNECTING
    val isConnected = state.status == com.ailivebear.app.tiktok.TikTokConnectionStatus.CONNECTED
    ...
    Column {
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            Button(
                onClick = {
                    if (isConnected || isBusy) viewModel.disconnect() else viewModel.connect(username)
                },
                enabled = username.isNotBlank()
            ) {
                Text(if (isConnected || isBusy) "Putuskan" else "Sambungkan")
            }
            Spacer(Modifier.width(12.dp))
            Text(statusLabel(state.status), style = MaterialTheme.typography.bodyMedium)
        }

        if (state.recentComments.isNotEmpty()) {
            Spacer(Modifier.height(10.dp))
            Column {
                state.recentComments.forEach { comment ->
                    Text(
...
private fun statusLabel(status: com.ailivebear.app.tiktok.TikTokConnectionStatus): String = when (status) {
    com.ailivebear.app.tiktok.TikTokConnectionStatus.DISCONNECTED -> "Tidak tersambung"
    com.ailivebear.app.tiktok.TikTokConnectionStatus.CONNECTING -> "Menyambungkan..."
    com.ailivebear.app.tiktok.TikTokConnectionStatus.CONNECTED -> "Terhubung"
    com.ailivebear.app.tiktok.TikTokConnectionStatus.RECONNECTING -> "Terputus - mencoba lagi..."
    com.ailivebear.app.tiktok.TikTokConnectionStatus.ERROR -> "Gagal menyambung"
}
